...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "swanlab" in default_backend:
            ...
            record_file = os.environ.get("SWANLAB_RUN_ID_RECORD_FILE")
            if record_file:
                try:
                    run = swanlab.get_run()
                    ...
                    if run_id:
                        ...
                        record_path.write_text(f"{run_id}\n", encoding="utf-8")
                        logger.info("Recorded SwanLab run id %s to %s", run_id, record_path)
                except Exception as exc:
                    logger.warning("Failed to record SwanLab run id to %s: %s", record_file, exc)
...
class FileLogger:
    def __init__(self, project_name: str, experiment_name: str):
        ...
        append_mode = os.getenv("VERL_FILE_LOGGER_APPEND", "").lower() in ("1", "true", "yes", "on")
        file_mode = "ab" if append_mode else "wb"
        if append_mode and os.path.exists(self.filepath):
            try:
                with open(self.filepath, "rb") as existing_fp:
                    for line in existing_fp:
                        ...
                        if isinstance(step, int):
                            self.max_logged_step = max(self.max_logged_step, step)
            except OSError as exc:
                logger.warning("Failed to scan existing file logger history at %s: %s", self.filepath, exc)
        print(f"Creating file logger at {os.path.abspath(self.filepath)} (mode={file_mode})")
        self.fp = open(self.filepath, file_mode, buffering=0)

    def log(self, data, step):
        if isinstance(step, int) and step <= self.max_logged_step:
            return
        data = {"step": step, "data": data}
        self.fp.write(orjson.dumps(data, option=orjson.OPT_SERIALIZE_NUMPY) + b"\n")
        if isinstance(step, int):
            self.max_logged_step = step
