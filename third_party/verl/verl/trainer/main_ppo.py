...
def run_ppo(config, task_runner_class=None) -> None:
    ...
    if not ray.is_initialized():
        ...
        runtime_env = {key: value for key, value in runtime_env.items() if value is not None}
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, is_train=True, max_samples: int = -1):
    ...
    if not is_train:
        val_apply_chat_template_kwargs = data_config.get("val_apply_chat_template_kwargs", None)
        if val_apply_chat_template_kwargs is not None:
            dataset_config = OmegaConf.create(OmegaConf.to_container(data_config, resolve=True))
            apply_chat_template_kwargs = dict(dataset_config.get("apply_chat_template_kwargs", {}))
            if OmegaConf.is_config(val_apply_chat_template_kwargs):
                val_apply_chat_template_kwargs = OmegaConf.to_container(val_apply_chat_template_kwargs, resolve=True)
            else:
                val_apply_chat_template_kwargs = dict(val_apply_chat_template_kwargs)
            apply_chat_template_kwargs.update(val_apply_chat_template_kwargs)
            dataset_config.apply_chat_template_kwargs = apply_chat_template_kwargs
