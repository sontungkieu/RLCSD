...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if "actor" in self.role:
            ...
            # Self-distillation: initialize teacher shadow (EMA/snapshot)
            self._teacher_shadow = None
            self._teacher_mode = actor_config.policy_loss.get("teacher_mode", "fixed")
            self._teacher_ema_decay = float(actor_config.policy_loss.get("ema_decay", 0.95))
            self._teacher_sync_interval = int(actor_config.policy_loss.get("teacher_sync_interval", 10))
            ...
            if self._teacher_mode in ("ema", "snapshot"):
                ...
                if torch.distributed.get_rank() == 0:
                    n = len(self._teacher_shadow)
                    mb = sum(p.numel() * p.element_size() for p in self._teacher_shadow.values()) / 1e6
                    print(f"[Self-Distill] Teacher shadow: mode={self._teacher_mode}, "
                          f"params={n}, size={mb:.1f}MB")
    ...
    def compute_log_prob(self, data: TensorDict) -> TensorDict:
        # Self-distillation: swap to teacher (EMA/snapshot) weights if requested
        use_teacher = tu.pop(data, key="use_teacher_shadow", default=False)
        if use_teacher and self._teacher_shadow is not None:
    ...
    def update_actor(self, data: TensorDict) -> TensorDict:
        ...
        # Update teacher shadow after training
        if self._teacher_shadow is not None:
